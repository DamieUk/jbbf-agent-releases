...
export class ServerHealthCheckService {
  ...
  async waitUntilServerReady(href: string, intervalValue = 3000, attempts = 100): Promise<void> {
    ...
    return of({}).pipe(
      ...
      tap(isReady => {
        ...
        if (currentAttempt === attempts) {
          throw new Error('Error: server health check polling attempts has reached');
        }
      }),
      delay(intervalValue),
      repeat(),
