      ...
      if (!err) {
        ...
        if (vmId) {
          try {
            ...
            await isFileExist(privateKeyPath)
              .catch(() =>
                writeFile(privateKeyPath, pems.privateKey)
                  .catch(err => logger.error(`Error while creating private key file --->>> `, err))
              )

            await isFileExist(certificatePath)
              .catch(() =>
                writeFile(certificatePath, pems.cert)
                  .catch(err => logger.error(`Error while creating certificate key file --->>> `, err))
              )
            ...
            await isFileExist(pfxKeyPath).catch(() => writeFile(pfxKeyPath, p12Der, {encoding: 'binary'})
