...
const createFolders = async () => {
  ...
  return isFileExist(PROJECT_LOGS_MAIN_PATH).catch(() =>
    writeFile(PROJECT_LOGS_MAIN_PATH, '')
  );
};

const runApp = async () => {
  logger.info('App is started and currently initializing...');
  ...
  const pollingInterval = 60000; /* 10 sec */
  const pollingAttempts = 100;
  try {
    await healthCheckService.waitUntilServerReady(
      healthCheckUrl,
      pollingInterval,
      pollingAttempts
    );
  ...
  // @ts-ignore
  const stopCheckingLaunchAutoUpdater = await launchAutoUpdater(ENV_VARS);
  await registerAgentWithVMWare(ENV_VARS);
};

const app = express();

app.listen(4000, runApp);
